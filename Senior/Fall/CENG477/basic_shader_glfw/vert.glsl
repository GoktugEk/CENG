...
void main(void)
{
	...
	vec4 p = gl_ModelViewMatrix * gl_Vertex; // translate to world coordinates

	vec3 L = normalize(lightPos - vec3(p));
	vec3 V = normalize(eyePos - vec3(p));
	vec3 H = normalize(L + V);
	vec3 N = vec3(gl_ModelViewMatrixInverseTranspose * vec4(gl_Normal, 0)); // provided by the programmer
	N = normalize(N);
	float NdotL = dot(N, L);
	float NdotH = dot(N, H);
