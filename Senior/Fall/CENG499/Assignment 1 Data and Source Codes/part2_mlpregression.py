...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    o1 = torch.matmul(input_data.float(),w1.float()) + b1
    o1 = torch.sigmoid(o1)
    o2 = torch.matmul(o1.float(),w2.float()) + b2
    return o2    
...
w1 = torch.from_numpy(np.random.normal(0, 1, (2, 32)))
...
b1 = torch.from_numpy(np.random.normal(0,1,(32)))
...
w2 = torch.from_numpy(np.random.normal(0, 1, (32, 1)))
...
for iteration in range(1, ITERATION+1):
    ...
    train_mse_loss = torch.mean((torch.sub(train_label,train_predictions))**2)
    ...
    with torch.no_grad():
        ...
        validation_mse_loss = torch.mean((torch.sub(validation_label,validation_predictions))**2)
...
with torch.no_grad():
    ...
    test_loss = torch.mean((torch.sub(test_label,test_predictions))**2)
